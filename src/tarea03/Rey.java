...
/**
 * Esta clase define color y posición de un rey mediante dos atributos. Un
 * atributo tipo enumerado "Color" y una instancia "posición" de la clase
 * "Posicion". Dispone de metodos "getter", "toString" y el metodo "Mueve" que
 * actualiza la posición de las instancias creadas.
 */
public class Rey {
    ...
    public void Mueve(Direccion direccion) {
        ...
        switch (direccion) {
            case NORTE:
                ...
                break;
            case SUR:
                if (Ilegal(Direccion.SUR)) {
                    posicion.setFila(fila - 1);
                }
                break;
            case ESTE:
                if (Ilegal(Direccion.ESTE)) {
                    posicion.setColumna((char) (numero_columna + 1));
                }
                break;
            case OESTE:
                if (Ilegal(Direccion.OESTE)) {
                    posicion.setColumna((char) (numero_columna - 1));
                }
                break;
            case NORESTE:
                if (Ilegal(Direccion.NORESTE)) {
                    posicion.setFila(fila + 1);
                    posicion.setColumna((char) (numero_columna + 1));
                }
                break;
            case SURESTE:
                if (Ilegal(Direccion.SURESTE)) {
                    posicion.setFila(fila - 1);
                    posicion.setColumna((char) (numero_columna + 1));
                }
                break;
            case NOROESTE:
                if (Ilegal(Direccion.NOROESTE)) {
                    posicion.setFila(fila + 1);
                    posicion.setColumna((char) (numero_columna - 1));
                }
                break;
            case SUROESTE:
                if (Ilegal(Direccion.SUROESTE)) {
                    posicion.setFila(fila - 1);
                    posicion.setColumna((char) (numero_columna - 1));
                }
                break;
